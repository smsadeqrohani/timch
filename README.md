...
// SMS Management
"sms:view"                       // Access SMS dashboard tab
"sms:send"                       // Send manual SMS messages
```
...
- **Automatic triggers**
  - هنگام ایجاد سفارش جدید پیام «مشتری گرامی...» برای مشتری ارسال می‌شود.
  - بعد از ثبت پرداخت نقدی پیام «سفارش شما ثبت شد (نقدی)» ارسال می‌گردد.
  - بعد از ایجاد قرارداد اقساط، پیام شامل مبلغ پیش‌پرداخت و اطلاعات اولین قسط ارسال می‌شود.
- **مدیریت پیامک‌ها**
  - تب جدید «پیامک‌ها» گزارش آخرین ۲۰۰ پیامک را نمایش می‌دهد (وضعیت، متن، گیرنده، رویداد).
  - در همین صفحه امکان ارسال فوری پیام (در صورت داشتن مجوز `sms:send`) فراهم است.
- **پیکربندی**
  - متغیرهای زیر باید در `.env.local` (و برای استقرار در Convex) تنظیم شوند:
    ```env
    KAVENEGAR_API_KEY=your-kavenegar-api-key
    KAVENEGAR_SENDER=10002440
    ```
  - برای محیط‌های غیرمحلی، از دستور زیر استفاده کنید:
    ```bash
    npx convex env set KAVENEGAR_API_KEY "your-kavenegar-api-key"
    npx convex env set KAVENEGAR_SENDER "10002440"
    ```
